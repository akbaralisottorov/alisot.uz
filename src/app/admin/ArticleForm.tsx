...
export default function ArticleForm({ initialData, onSuccess }: ArticleFormProps) {
  ...
  // Sync formData when initialData changes (edit mode)
  useEffect(() => {
  ...
  // Fetch users list for the author dropdown
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          // Auto-select first user when creating
  ...
  const handleImageUpload = async (file: File) => {
    ...
    const res = await fetch(API_ROUTES.upload, { method: "POST", body: uploadData });
    if (!res.ok) throw new Error("Rasm yuklashda xatolik");
    const data = await res.json();
    handleField("coverImage", data.url as string);
  };
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-4xl mx-auto space-y-8 pb-32">
      ...
      <div className="flex items-center justify-between bg-card/60 border border-border/60 rounded-2xl p-4 px-6 sticky top-24 z-10 backdrop-blur-md shadow-sm">
        ...
        <Button
          type="submit"
          disabled={loading}
          className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-2xl px-8 h-10 font-bold shadow-md shadow-primary/20"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-16 pt-16 border-t border-border/40">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-xs text-muted-foreground font-bold">Muqova rasmi</Label>
          ...
          {users.length > 0 && (
            <div className="space-y-2">
              ...
              <Select value={formData.authorId} onValueChange={(val) => handleField("authorId", val)}>
                <SelectTrigger className="bg-card/40 border-border/60 rounded-xl h-10">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Qidiruv natijasida chiqadigan tavsif"
            />
          </div>
        </div>
      </div>
